...
again:
	...
	new = device_create(vfio.class, dev, group->devt, group,
			    "%s:%u", dev->bus->name, groupid);
	if (IS_ERR(new)) {
		kfree(group);
		idr_remove(&vfio.idr, minor);
		return (struct vfio_group *)new;
	}
...
int vfio_group_add_dev(struct device *dev, const struct vfio_device_ops *ops)
{
	...
	if (!group) {
		group = __vfio_create_group(dev, groupid);
		if (IS_ERR(group)) {
			ret = PTR_ERR(group);
			goto out;
		}
	...
	if (!device) {
		if (__vfio_group_devs_inuse(group) ||
			...
			printk(KERN_WARNING
			       "Adding device %s to group %s/%u while group is already in use!!\n",
		...
		if (WARN_ON(!device)) {
			 ...
			 * taint the group to prevent it being used. */
			group->tainted = true;
			ret = -ENOMEM;
			goto out;
		}
		...
		if (group->iommu && group->iommu->domain) {
			ret = __vfio_iommu_attach_dev(group->iommu, device);
			if (WARN_ON(ret)) {
...
out:
	/* If we weren't able to find or allocate a group, then we
	 * can't even taint it, which means there's an untracked device
	 * that should belong to a group. */
	BUG_ON(!group);
...
static char *vfio_devnode(struct device *dev, mode_t *mode)
{
	...
	if(WARN_ON(iommu_device_group(dev->parent, &groupid)))
		return NULL;

	return kasprintf(GFP_KERNEL, "vfio/%s/%u",
			 dev->parent->bus->name, groupid);
}
