 ...
 */
int vfio_pci_build_config_map(struct vfio_pci_device *vdev)
{
	struct pci_dev *pdev = vdev->pdev;
	u8 *map;
	int i, len;
	...
	while (pos && --loops > 0) {
		...
		if (cap > PCI_CAP_ID_MAX) {
			printk(KERN_WARNING
				"%s: unknown pci capability id 0x%x\n",
				__func__, cap);
			len = 0;
		} else
		...
		if (len == 0) {
			printk(KERN_WARNING
				"%s: unknown length for pci cap 0x%x\n",
				__func__, cap);
			len = PCI_CAP_SIZEOF;
		}
		if (len == 0xFF) {	/* variable field */
			...
			case PCI_CAP_ID_SATA:
				ret = pci_read_config_byte(pdev,
					pos + PCI_SATA_REGS, &tmp);
				if (ret < 0)
					return ret;
				tmp &= PCI_SATA_REGS_MASK;
				if (tmp == PCI_SATA_REGS_INLINE)
	...
	while (loops-- > 0) {
		...
		if (ecap > PCI_EXT_CAP_ID_MAX) {
			printk(KERN_WARNING
				"%s: unknown pci ext capability id 0x%x\n",
				__func__, ecap);
			len = 0;
		} else
		...
		if (len == 0xFF) {	/* variable field */
			...
			case PCI_EXT_CAP_ID_REBAR:
				...
				if (ret)
					break;
				len = 4;
				tmp = (tmp & PCI_REBAR_CTRL_NBAR_MASK)
			...
			case PCI_EXT_CAP_ID_TPH:
				...
				flags = dw & PCI_TPH_CAP_LOC_MASK;
				if (flags == PCI_TPH_LOC_CAP) {
					int sts;

					sts = (dw & PCI_TPH_CAP_ST_MASK)
		...
		if (len == 0 || len == 0xFF) {
			printk(KERN_WARNING
				"%s: unknown length for pci ext cap 0x%x\n",
				__func__, ecap);
			len = PCI_CAP_SIZEOF;
		}
		for (i = 0; i < len; i++) {
 ...
 */
static int vfio_virt_init(struct vfio_pci_device *vdev)
{
	struct pci_dev *pdev = vdev->pdev;
	u32 *lp;
	u16 *wp;
	...
	/* Create a copy of config space.  Note pci_read_config_* returns
	 * native endian, so we need to convert to little endian for an
	 * identical copy. */
	lp = (u32 *)vdev->vconfig;
	...
	/* for sr-iov devices */
	wp = (u16 *)&vdev->vconfig[PCI_VENDOR_ID];
	*wp = cpu_to_le16(pdev->vendor);
	wp++;
	*wp = cpu_to_le16(pdev->device);
...
static inline int vfio_read_config_byte(struct vfio_pci_device *vdev,
					int pos, u8 *valp)
{
	return pci_user_read_config_byte(vdev->pdev, pos, valp);
}

static inline int vfio_write_config_byte(struct vfio_pci_device *vdev,
					int pos, u8 val)
{
...
static void vfio_virt_basic(struct vfio_pci_device *vdev, int write, u16 pos, u8 *rbp)
{
	u8 val;
 ...
 */
static void vfio_virt_msi(struct vfio_pci_device *vdev, int write,
...
ssize_t vfio_pci_config_readwrite(int write,
		struct vfio_pci_device *vdev,
		char __user *buf,
		size_t count,
		loff_t *ppos)
{
	...
	if (!vdev->pci_config_map) {
		ret = vfio_pci_build_config_map(vdev);
		if (ret)
			goto out;
	}
	if (!vdev->vconfig) {
