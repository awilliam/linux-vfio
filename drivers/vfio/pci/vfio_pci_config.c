...
static int vfio_cap_len(struct vfio_pci_device *vdev, u8 cap, u8 pos)
{
	...
	default:
		printk(KERN_WARNING "%s: %s unknown length for pci cap "
...
static int vfio_ext_cap_len(struct vfio_pci_device *vdev, u16 ecap, u16 epos)
{
	...
	default:
		printk(KERN_WARNING "%s: %s unknown length for pci ecap 0x%x"
...
static int vfio_cap_init(struct vfio_pci_device *vdev)
{
	...
	while (pos && loops--) {
		...
		for (i = 0; i < len; i += 4) {
			...
			printk(KERN_WARNING "%s: %s pci config conflict @0x%x,"
			       " was cap 0x%x now cap 0x%x\n", __func__,
			       dev_name(&pdev->dev), pos + i,
			       map[pos + i], cap);
		}
...
static int vfio_ecap_init(struct vfio_pci_device *vdev)
{
	...
	while (loops-- && epos >= PCI_CFG_SPACE_SIZE) {
		...
		for (i = 0; i < len; i += 4) {
			...
			printk(KERN_WARNING "%s: %s pci config conflict @0x%x,"
			       " was ecap 0x%x now ecap 0x%x\n", __func__,
			       dev_name(&pdev->dev), epos + i, map[epos + i],
			       ecap);
		}
