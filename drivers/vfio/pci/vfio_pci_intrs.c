 ...
 */
irqreturn_t vfio_pci_disable_intx(struct vfio_pci_device *vdev)
{
...
irqreturn_t vfio_pci_interrupt(int irq, void *dev_id)
{
	struct vfio_pci_device *vdev = dev_id;
	irqreturn_t ret = vfio_pci_disable_intx(vdev);
...
int vfio_pci_setup_msi(struct vfio_pci_device *vdev, int nvec, int __user *intargp)
{
	struct pci_dev *pdev = vdev->pdev;
...
int vfio_pci_setup_msix(struct vfio_pci_device *vdev, int nvec, int __user *intargp)
{
	struct pci_dev *pdev = vdev->pdev;
	struct eventfd_ctx *ctx;
	int ret = 0;
